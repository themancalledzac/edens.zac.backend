...
public class WebAuthnService {
   ...
   *
   * <p>The account must pass {@link SessionService#mayHoldSession}. A verified assertion proves
   * possession of the credential, not that the account may still log in -- a passkey outlives any
   * status change, so status is read here rather than inferred from the ceremony succeeding. The
   * test goes through the predicate rather than comparing to a status literal, so a new {@code
   * UserStatus} is admitted or refused here by the same rule that governs {@link
   ...
   *     assertion may not hold a session
   */
  public String finishLogin(
    ...
    if (!SessionService.mayHoldSession(user.getStatus())) {
