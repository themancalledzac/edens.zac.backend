...
/**
 * Consolidated request and response DTOs for Collection operations. Uses Java
 * records for
...
public final class CollectionRequests {
    ...
    /**
     * Minimal request for creating a new ContentCollection. Only accepts the
     * essentials: type and
     * title.
     */
    ...
    public record Update(
            ...
            PersonUpdate people,
            /**
             * Collection updates using prev/new/remove pattern. Used to manage child
             * collections within
    ...
    /**
     * Request for reordering content within a collection. Allows multiple content
     * items to be
     * reordered in a single atomic operation. Works with any content type: IMAGE,
     * TEXT, GIF,
     * COLLECTION.
     */
    public record Reorder(
        ...
        public record ReorderItem(
                /**
                 * The content ID (from content[].id in the collection response). This is the
                 * content table
    ...
    /**
     * Response DTO for the collection update/manage endpoint. Contains the
     * collection along with all
     * metadata needed for the update UI. Uses composition with GeneralMetadataDTO
     * to avoid field
     * duplication.
     */
    public record UpdateResponse(
            ...
            /**
             * General metadata including tags, people, cameras, lenses, film types, film
             * formats, and
             * collections. This is unwrapped during JSON serialization to maintain
             * backwards
    ...
    /**
     * Location update wrapper using prev/new/remove pattern. All fields are
     * optional to support
     * partial updates.
     *
     * <p>
     * - prev: ID of existing location to use - newValue: Name of new location to
     * create - remove:
     * true to remove location association
     *
     * <p>
     * Examples: - {prev: 5} = Use existing location ID 5 - {newValue: "New York"} =
     * Create new
    ...
    /**
     * Tag update wrapper using prev/new/remove pattern. All fields are optional to
     * support partial
     * updates.
     *
     * <p>
     * - prev: List of existing tag IDs to keep/add - newValue: List of new tag
     * names to create
     * and add - remove: List of tag IDs to remove
     *
     * <p>
     * Examples: - {prev: [2, 3]} = Add existing tags 2 and 3 - {newValue:
     * ["landscape",
     * "nature"]} = Create and add new tags - {remove: [1]} = Remove tag ID 1 -
     * {prev: [2], newValue:
    ...
    /**
     * Person update wrapper using prev/new/remove pattern. All fields are optional
     * to support
     * partial updates.
     *
     * <p>
     * - prev: List of existing person IDs to keep/add - newValue: List of new
     * person names to
     * create and add - remove: List of person IDs to remove
     *
     * <p>
     * Examples: - {prev: [5, 6]} = Add existing people 5 and 6 - {newValue: ["John
     * Doe"]} =
     * Create and add new person - {remove: [3]} = Remove person ID 3 - {prev: [5],
     * newValue:
     * ["Jane"], remove: [3]} = All operations at once
     */
    public record PersonUpdate(List<Long> prev, List<String> newValue, List<Long> remove) {
    }

    /**
     * Collection update wrapper using prev/new/remove pattern for managing child
     * collections within
     * a parent collection. All fields are optional to support partial updates.
     *
     * <p>
     * - prev: Update existing child collections (visibility/order) - newValue: Add
     * new child
     * collections to the parent - remove: IDs of child collections to remove from
     * the parent
     *
     * <p>
     * The remove field accepts either: - The content ID
     * (ContentCollectionEntity.id) - matches
     * the "id" field in API responses - The referenced collection ID
     * (referencedCollectionId) -
     * matches the "referencedCollectionId" field
     *
     * <p>
     * Examples: - {prev: [{collectionId: 1, visible: true}]} = Update visibility of
     * child
     * collection - {newValue: [{collectionId: 2, visible: true, orderIndex: 5}]} =
     * Add child
     * collection at position 5 - {remove: [3]} = Remove child collection with ID 3
     * from parent
     *
     * <p>
     * Note: For reordering content within a collection, use the dedicated reorder
     * endpoint
     * instead.
     */
    public record CollectionUpdate(
            List<Records.ChildCollection> prev, List<Records.ChildCollection> newValue, List<Long> remove) {
    }
}
