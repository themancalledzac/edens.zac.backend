...
/**
 * DAO for ContentCollectionEntity using raw SQL queries.
 * Handles JOINED inheritance pattern (content + content_collection).
 */
@Component
@Slf4j
public class ContentCollectionDao extends BaseDao {
     ...
     */
    @Transactional(readOnly = true)
    public Optional<ContentCollectionEntity> findByReferencedCollectionId(Long referencedCollectionId) {
        String sql = SELECT_CONTENT_COLLECTION + " WHERE cc.referenced_collection_id = :referencedCollectionId";
    ...
    /**
     * Save ContentCollectionEntity.
     * Inserts into content first, then content_collection using the same ID.
     */
    @Transactional
    public ContentCollectionEntity save(ContentCollectionEntity entity) {
        ...
        if (entity.getId() == null) {
            ...
            MapSqlParameterSource collectionParams = createParameterSource()
