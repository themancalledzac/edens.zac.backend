...
public class ContentCameraDao extends BaseDao {
  ...
  // Row mapper for queries that don't need body_serial_number (most queries)
  private static final RowMapper<ContentCameraEntity> CAMERA_ROW_MAPPER = (rs, rowNum) -> {
  ...
  public Optional<ContentCameraEntity> findByBodySerialNumber(String bodySerialNumber) {
    if (bodySerialNumber == null || bodySerialNumber.trim().isEmpty()) {
      return Optional.empty();
    }
    String sql = "SELECT id, camera_name, body_serial_number, created_at FROM content_cameras WHERE body_serial_number = :bodySerialNumber";
    MapSqlParameterSource params = createParameterSource().addValue("bodySerialNumber", bodySerialNumber.trim());
  ...
  public Optional<ContentCameraEntity> findByCameraName(String cameraName) {
    String sql = "SELECT id, camera_name, created_at FROM content_cameras WHERE camera_name = :cameraName";
  ...
  public Optional<ContentCameraEntity> findByCameraNameIgnoreCase(String cameraName) {
    String sql = "SELECT id, camera_name, created_at FROM content_cameras WHERE LOWER(camera_name) = LOWER(:cameraName)";
    MapSqlParameterSource params = createParameterSource().addValue("cameraName", cameraName);
    return queryForObject(sql, CAMERA_ROW_MAPPER, params);
  }
  ...
  public List<ContentCameraEntity> findByCameraNameContainingIgnoreCase(String searchTerm) {
    String sql = "SELECT id, camera_name, created_at FROM content_cameras WHERE LOWER(camera_name) LIKE LOWER(:searchTerm) ORDER BY camera_name ASC";
  ...
  public boolean existsByCameraNameIgnoreCase(String cameraName) {
    String sql = "SELECT COUNT(*) > 0 FROM content_cameras WHERE LOWER(camera_name) = LOWER(:cameraName)";
    MapSqlParameterSource params = createParameterSource().addValue("cameraName", cameraName);
    Boolean result = namedParameterJdbcTemplate.queryForObject(sql, params, Boolean.class);
    return result != null && result;
  }
  ...
  public ContentCameraEntity save(ContentCameraEntity entity) {
    if (entity.getId() == null) {
      String sql = "INSERT INTO content_cameras (camera_name, body_serial_number, created_at) VALUES (:cameraName, :bodySerialNumber, :createdAt)";
