...
/**
 * DAO for ContentEntity and its subclasses using raw SQL queries.
 * Handles JOINED inheritance pattern (content + content_image, content_text, etc.).
 * Replaces ContentRepository.
 */
@Component
@Slf4j
public class ContentDao extends BaseDao {
    ...
    /**
     * RowMapper for ContentImageEntity (JOINED inheritance).
     * Joins content and content_image tables.
     */
    private static final RowMapper<ContentImageEntity> CONTENT_IMAGE_ROW_MAPPER = (rs, rowNum) -> {
        ContentImageEntity entity = ContentImageEntity.builder()
            ...
            .filmFormat(rs.getString("film_format") != null ? 
                edens.zac.portfolio.backend.types.FilmFormat.valueOf(rs.getString("film_format")) : null)
        ...
        // Note: Foreign key relationships (camera, lens, filmType) loaded separately if needed
        Long cameraId = getLong(rs, "camera_id");
        if (cameraId != null) {
            entity.setCamera(edens.zac.portfolio.backend.entity.ContentCameraEntity.builder().id(cameraId).build());
        }
        
        Long lensId = getLong(rs, "lens_id");
        if (lensId != null) {
            entity.setLens(edens.zac.portfolio.backend.entity.ContentLensEntity.builder().id(lensId).build());
        }
        
        Long filmTypeId = getLong(rs, "film_type_id");
        if (filmTypeId != null) {
            entity.setFilmType(edens.zac.portfolio.backend.entity.ContentFilmTypeEntity.builder().id(filmTypeId).build());
        }
        
        return entity;
    };

    private static final RowMapper<edens.zac.portfolio.backend.entity.ContentTextEntity> CONTENT_TEXT_ROW_MAPPER = (rs, rowNum) ->
    ...
    private static final RowMapper<edens.zac.portfolio.backend.entity.ContentGifEntity> CONTENT_GIF_ROW_MAPPER = (rs, rowNum) ->
    ...
    private static final String SELECT_CONTENT_IMAGE = """
        SELECT c.id, c.content_type, c.created_at, c.updated_at,
               ...
               ci.film_format, ci.shutter_speed, ci.camera_id, ci.focal_length, ci.location,
    ...
    /**
     * Find all images ordered by createDate DESC.
     * Note: Relationships (tags, people, camera, lens, filmType) loaded separately.
     */
    @Transactional(readOnly = true)
    public List<ContentImageEntity> findAllImagesOrderByCreateDateDesc() {
        String sql = SELECT_CONTENT_IMAGE + 
            " ORDER BY ci.create_date DESC NULLS LAST, c.created_at DESC";
    ...
    /**
     * Find all ContentEntity by IDs, loading the fully-typed entities.
     * Returns ContentImageEntity, ContentTextEntity, ContentGifEntity, or ContentCollectionEntity
     * based on each entity's content_type.
     *
    ...
    public List<ContentEntity> findAllByIds(List<Long> ids) {
        ...
        if (idsByType.containsKey(ContentType.IMAGE)) {
            ...
            MapSqlParameterSource imageParams = createParameterSource()
                .addValue("ids", idsByType.get(ContentType.IMAGE));
     ...
     */
    @Transactional(readOnly = true)
    public Optional<ContentCollectionEntity> findContentCollectionByReferencedCollectionId(Long referencedCollectionId) {
        String sql = """
    ...
    /**
     * Save ContentImageEntity.
     * IMPORTANT: Must be wrapped in @Transactional.
     * Inserts into content first, then content_image using the same ID.
     */
    @Transactional
        ...
        if (entity.getId() == null) {
            ...
            String imageSql = """
                INSERT INTO content_image (id, title, image_width, image_height, iso, author, rating,
                                          ...
                                          film_format, shutter_speed, camera_id, focal_length, location,
                ...
                VALUES (:id, :title, :imageWidth, :imageHeight, :iso, :author, :rating,
                        ...
                        :filmFormat, :shutterSpeed, :cameraId, :focalLength, :location,
            ...
            MapSqlParameterSource imageParams = createParameterSource()
                ...
                .addValue("filmFormat", entity.getFilmFormat() != null ? entity.getFilmFormat().name() : null)
            ...
            String imageSql = """
                ...
                SET title = :title, image_width = :imageWidth, image_height = :imageHeight, iso = :iso,
                    ...
                    focal_length = :focalLength, location = :location, image_url_web = :imageUrlWeb,
                    image_url_original = :imageUrlOriginal, create_date = :createDate, file_identifier = :fileIdentifier
                WHERE id = :id
                """;
    ...
    /**
     * Save image tags (many-to-many relationship).
     * Deletes existing tags and inserts new ones.
     */
    @Transactional
    public void saveImageTags(Long imageId, List<Long> tagIds) {
        // Delete existing tags
        String deleteSql = "DELETE FROM content_image_tags WHERE image_id = :imageId";
        MapSqlParameterSource deleteParams = createParameterSource().addValue("imageId", imageId);
        update(deleteSql, deleteParams);
        
        // Insert new tags
        if (tagIds != null && !tagIds.isEmpty()) {
            String insertSql = "INSERT INTO content_image_tags (image_id, tag_id) VALUES (:imageId, :tagId)";
    ...
    /**
     * Save image people (many-to-many relationship).
     * Deletes existing people and inserts new ones.
     */
    @Transactional
    public void saveImagePeople(Long imageId, List<Long> personIds) {
        ...
        if (personIds != null && !personIds.isEmpty()) {
            ...
            MapSqlParameterSource[] batchParams = personIds.stream()
                .map(personId -> createParameterSource()
                    .addValue("imageId", imageId)
     ...
     */
    @Transactional(readOnly = true)
    public List<Long> findImageTagIds(Long imageId) {
        String sql = "SELECT tag_id FROM content_image_tags WHERE image_id = :imageId";
    ...
    /**
     * Delete ContentImageEntity by ID.
     * Deletes from content_image first (child table), then content (parent table).
     * Also deletes related tags and people associations.
     */
    @Transactional
    public void deleteImageById(Long id) {
        ...
        String deleteTagsSql = "DELETE FROM content_image_tags WHERE image_id = :id";
