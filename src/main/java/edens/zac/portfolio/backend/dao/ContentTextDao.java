...
/**
 * DAO for ContentTextEntity using raw SQL queries.
 * Handles JOINED inheritance pattern (content + content_text).
 */
@Component
@Slf4j
public class ContentTextDao extends BaseDao {
    ...
    private static final RowMapper<ContentTextEntity> CONTENT_TEXT_ROW_MAPPER = (rs, rowNum) -> {
    ...
    /**
     * Save ContentTextEntity.
     * Inserts into content first, then content_text using the same ID.
     */
    @Transactional
    public ContentTextEntity save(ContentTextEntity entity) {
        ...
        if (entity.getId() == null) {
            ...
            MapSqlParameterSource textParams = createParameterSource()
