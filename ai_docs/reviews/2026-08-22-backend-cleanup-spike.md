...
- [ ] **`CollectionRepository` and `CollectionRepositoryTest` inline comments -- ~107 lines.**
  *(Filed 2026-08-31, sixth run.)* [#280](https://github.com/themancalledzac/edens.zac.backend/pull/280) edited both files and deliberately left their
  comments, on the grounds that clearing them would triple that PR's diff. That call follows the
  rule-37 row's own "do not sweep this in one MR -- take it per package" and the [#272](https://github.com/themancalledzac/edens.zac.backend/pull/272)
  precedent, and contradicts rule 37 read literally. **Both readings were exercised in the same run**:
  #280 left them while [#282](https://github.com/themancalledzac/edens.zac.backend/pull/282), [#283](https://github.com/themancalledzac/edens.zac.backend/pull/283) and [#285](https://github.com/themancalledzac/edens.zac.backend/pull/285) each deleted the banners in
  the files they touched. Rule 37 now carries the boundary that settles it. This is the leftover
  concentration, and it is its own MR. Re-measure before starting -- the ~107 figure is #280's count,
  not a re-derivation. **COLD.**
- [ ] **#12b. The two `nextOrderIndex` copies [#284](https://github.com/themancalledzac/edens.zac.backend/pull/284) deliberately left in `CollectionService`.**
  *(Filed 2026-08-31, sixth run, out of MR 18 #12's implementation.)* #284 collapsed three copies onto
  ...
  cut from that PR with reasons: `CollectionService.linkCollectionToParent` (**~444**) and the
  child-collection add path (**~1141**) -- **find both by name**, this file moves constantly. Neither
...
nine PRs were open when it was written. [#277](https://github.com/themancalledzac/edens.zac.backend/pull/277) also edits this tracker from outside the run,
so whichever of the two merges second resolves by hand. The inline-comment count was deliberately
**not** re-run for this reason; see that row.
